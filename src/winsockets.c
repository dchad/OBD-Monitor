/* 
   ...
   Description: Winsock version of UDP server and client functions. 
...
int init_server_socket(char *server, char *port)
{
   ...
   if (iResult != NO_ERROR)
      printf("init_server_comms() <ERROR>: ioctlsocket failed with error: %ld\n", iResult);
...
int recv_ecu_msg(char *msg)
{
   ...
   n = recvfrom(sock,msg,256,0,(struct sockaddr *)&from,&length);
